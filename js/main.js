...
if (document.readyState != "loading") {
  randomPostWidth();
  if (document.getElementById("svgToShowMe") !== null) clickMeEvent();
  // rescroll();
}
// modern browsers
...
function randomPostWidth() {
  ...
  if (postList) {
    ...
    for (var i = 0; i < numberOfPosts; i++) {
      basisVal = randomIntFromInterval(20, 35);
      eachPost[i].style["flex-basis"] = basisVal + "%";

      //seta a cor da borda de topo de cada post
      eachPost[i].firstElementChild.style.borderTopColor = get_random_color();
      eachPost[i].querySelector("h2").style.color =
        eachPost[i].firstElementChild.style.borderTopColor;
      ...
      eachPost[i].firstElementChild.addEventListener("mouseenter", function() {
        this.style.borderBottomColor = this.style.borderTopColor;
      });
    }
  }
}
...
$("body").on("click", ".btn-toggle-menu", function() {
  $(this).toggleClass(function() {
    return $(this).is(".menu-open, .menu-close")
...
//     //   );
//       return (
//         $(this).position().top > $(window).height() + $(window).scrollTop()
//       );
//     })
//     .hide();
// }
